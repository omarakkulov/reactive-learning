...
> Изначально мы хотели разобрать весь WebFlux Request Path, но тема оказалась слишком большой.
> Поэтому сегодня разбираем нижнюю часть пути запроса: от Netty server до EventLoop и Reactor Netty.

То есть, как мы обсуждали на прошлой лекции, Netty низкоуровнево работает с сырыми байтами данных, которые каким-то образом должны дойти
до Spring-Webflux слоя, а если точнее, то до слоя Reactor Netty, преобразующий эти байты в понятные Http-запросы приложению.
