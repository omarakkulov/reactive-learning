...
2) Как только Channel зарегистрирован в EventLoop, то я могу думать таким образом:

   ![cpu_threads.png](../../../../../../resources/static/lesson01/event_loop_working_2.png)

   Этот запрос преобразуется в задачу(task), или, правильнее, в эвент. Всякий раз, при регистрации эвента, он помещается в очередь,
   с которой работает EventLoop
3) Эвент, поступающий через созданный Channel, отправится в очередь задач подобным образом, и у каждого EventLoop будет собственная
   очередь задач
...
Большое спасибо всем, кто дослушал, кому было интересно!
На следующей лекции мы обязательно пройдем и проанализируем подробно весь путь HTTP-запроса в Webflux, чтобы понимать, как все работает
изнутри. По аналогии с блокирующим подходом в Spring MVC. То есть, мы увидим то, как создаются Channel, как работает EventLoop и
все-все-все на практике.
